...
namespace Migrator
{
    ...
    class Program
    {
        ...
        static string mongoConnectionString = "mongodb+srv://username:password@hostname/myFirstDatabase";
        ...
        static async Task Main(string[] args)
        {
            ...
            if (datePipelineResults.Count > 0)
            {
                ...
                var dataList = new List<ReplaceOneModel<Movie>>();
                foreach (Movie m in datePipelineResults)
                {
                    var filter = Builders<Movie>.Filter.Eq(doc => doc.Id, m.Id);
                    dataList.Add(new ReplaceOneModel<Movie>(filter, m));
                }
                bulkWriteDatesResult = await _moviesCollection.BulkWriteAsync(dataList, new BulkWriteOptions { IsOrdered = false });
            ...
            if (ratingPipelineResults.Count > 0)
            {
                ...
                var dataList = new List<ReplaceOneModel<Movie>>();
                foreach (Movie m in ratingPipelineResults)
                {
                    var filter = Builders<Movie>.Filter.Eq(doc => doc.Id, m.Id);
                    dataList.Add(new ReplaceOneModel<Movie>(filter, m));
                }
                bulkWriteRatingsResult = await _moviesCollection.BulkWriteAsync(dataList, new BulkWriteOptions{ IsOrdered = false});
